...
- **2026-09-11:** Back Navigation Deep Audit & CloseWatcher Singleton Stack (6 bugs fixed):
  ...
  - **2026-09-12:** Fix PWA Duplicate Push Notifications:
  - **The Problem:** When ZeoTask was installed as a PWA, task due notifications and digests appeared twice on the user's device (one labeled as Chrome/browser, one labeled as ZeoTask PWA).
  - **Root Cause 1 (Dual showNotification in Service Worker):** When FCM backend sends messages with a top-level `notification: { title, body }`, Firebase Messaging SDK's internal Service Worker listener automatically displays it. In `src/sw.ts`, `onBackgroundMessage` was also calling `self.registration.showNotification()`, creating a duplicate notification. Because the backend didn't specify `webpush.notification.icon`, Firebase SDK's first notification used the default browser icon (labeled as Chrome), while the second one had `/pwa-192.png` (labeled as ZeoTask).
  - **Root Cause 2 (Dual Token Subscriptions):** When visiting the site in Chrome browser, a token was stored in `users/{uid}/pushTokens`. When installed as a WebAPK/PWA, a new token was generated under the WebAPK package. Both tokens remained valid and active in Firestore, causing Cloud Functions to send push events to both Chrome and WebAPK.
  - **Root Cause 3 (Missing Notification Collision Tag):** Neither notification used a `tag`, preventing the browser/Android NotificationManager from collapsing duplicates.
  - **The Fix:**
    - **Service Worker (`src/sw.ts`):** In `onBackgroundMessage`, if `payload.notification` exists, bail out immediately to let Firebase SDK handle display. For data-only messages, enforce `tag` and `/pwa-192.png`. Added client focus check to suppress notifications when app is active. Enhanced `notificationclick` to support `fcmOptions.link`.
    - **Cloud Functions (`functions/src/index.ts`):** In `sendToUserWithCleanup`, added `webpush.notification` with `tag`, `icon: '/pwa-192.png'`, `badge: '/pwa-192.png'`, and `fcmOptions.link`. In `getTokensForUser`, implemented multi-token deduplication and pruning: groups by `deviceId` / normalized mobile `userAgent`, prioritizes `isStandalone: true` PWA tokens over browser tokens, keeps only the most recent token per physical device, and batch-deletes obsolete tokens from Firestore.
    - **Client Push (`src/lib/push.ts` & `AppShell.tsx`):** Added `zeotask_device_id` in `localStorage`. Saved `deviceId` and `isStandalone: isStandaloneDisplay()` on registration, deleting previous tokens for the same device. Exported `syncPushTokens(uid)` and invoked it on `AppShell` mount when notification permission is granted, automatically purging stale browser tokens when the user opens the PWA.
